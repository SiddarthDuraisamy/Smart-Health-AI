...
@router.post("/health-assessment", response_model=Dict[str, Any])
async def get_health_assessment(
    ...
    return {
        ...
        "next_assessment_due": datetime.utcnow().replace(month=datetime.utcnow().month + 3)  # 3 months
    }
...
async def chat_with_ai(
    ...
    if consultation_id and ai_response:
        try:
            consultations_collection = await get_consultations_collection()
