...
async def register_user(user_data: UserCreate):
    ...
    if user_data.role == UserRole.PATIENT:
        ...
        await patients_collection.insert_one(PatientInDB(**patient_data.dict()).dict(by_alias=True))
        ...
        await doctors_collection.insert_one(DoctorInDB(**doctor_data.dict()).dict(by_alias=True))
...
async def login_user(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    await users_collection.update_one(
        {"_id": user.id},
        {"$set": {"last_login": user_in_db.updated_at}}
    )
